    ...
    if (Test-Path $moduleRootPath) {
        ...
        if ($moduleDir) {
            ...
            if (Test-Path $psd1) {
                ...
                $module = Import-Module $psd1 -Force
                *AddTaskAliases $module
                return
            }
...
function *AddTaskAliases {
    param($Module)
    if ($Module -is [string]) {
        $Module = Get-Module -Name $Module
    }

    $path = Join-Path (Split-Path $Module.Path) 'InvokeBuildTasks'
    Get-ChildItem $path -ErrorAction SilentlyContinue |
        ForEach-Object {
            $taskPath = $_.FullName
            $name = [System.IO.Path]::GetFileNameWithoutExtension($_.Name)
            Set-Alias -Name $name -Value $taskPath
        }
}
